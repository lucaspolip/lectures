...
```{r}
library(quanteda)
tweets <- read.csv('candidate-tweets.csv', stringsAsFactors=F) #stringAsFactors=F because if it is true would return as a categorical variable and we want as a text.
```
...
```{r}
pos.words <- pos.words[-which(pos.words=="great")] ##great is part of trump campaign, therefore, we should take it out. 
